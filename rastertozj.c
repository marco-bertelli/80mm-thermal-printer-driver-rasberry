...
static const struct command cashDrawerEject = {5,
                                               (char[5]) {0x1b, 0x70, 0x00, 0x50, 0x50}};

// define raster mode start command
static const struct command rasterModeStartCommand = {4, (char[4]) {0x1d, 0x76, 0x30, 0}};

// Modified by Yiming Zhang on July 21, 2020
// define cut command
static const struct command cutPaper = {6, (char[6]) {0x0a, 0x1d, 0x56, 0x42, 0x01, 0x0a}};
...
static inline void initializeSettings(char *commandLineOptionSettings) {
    ...
    settings.cutMedia = getOptionChoiceIndex("Cutting", ppd);
...
int main(int argc, char *argv[]) {
    ...
    while (cupsRasterReadHeader2(ras, &header)) {
        ...
        // Modified by Yiming Zhang on Jul 21, 2020
        if (settings.cutMedia == 1) {
            outputCommand(cutPaper);
        }
